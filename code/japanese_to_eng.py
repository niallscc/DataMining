...
if __name__ == "__main__":
    # Enter the path to home folder containing the input files.
    path = '../input/'
    df = pd.read_excel(
        path + 'documentation/CAPSULE_TEXT_Translation.xlsx', skiprows=5)
    ...
    for f in files:
        ...
        df['CAPSULE_TEXT'] = [
            capsuleText[encode_text(x)] for x in df['CAPSULE_TEXT']]
        df['GENRE_NAME'] = [
            genreName[encode_text(x)] for x in df['GENRE_NAME']]
